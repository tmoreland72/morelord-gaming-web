...
export const POST: RequestHandler = async ({ request, platform, url }) => {
	...
	try {
		...
		return json({ ...activation, verificationUrl: `${url.origin}/account` });
	} catch (error) {
		return json({ error: error instanceof Error ? error.message : 'Activation could not be started.' }, { status: 400 });
	}
};
