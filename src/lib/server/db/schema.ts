...
export const productFeatures = sqliteTable(
	'product_features',
	{
		...
		tier: text('tier', { enum: ['standard', 'premium', 'champion'] })
			.notNull()
			.default('standard'),
...
export const tierFeatures = sqliteTable(
	'tier_features',
	{
		tierId: text('tier_id')
			.notNull()
...
export const subscriptions = sqliteTable(
	...
	(table) => [
		uniqueIndex('subscriptions_customer_idx').on(table.stripeCustomerId, table.stripeSubscriptionId)
	]
);

export const activeEntitlements = sqliteTable(
	'active_entitlements',
	{
		id: text('id').primaryKey(),
		stripeCustomerId: text('stripe_customer_id').notNull(),
	...
	(table) => [
		uniqueIndex('active_entitlements_customer_key_unique').on(
...
export const foundryProductActivity = sqliteTable(
	'foundry_product_activity',
	{
		installationId: text('installation_id')
			.notNull()
			.references(() => foundryInstallations.id, { onDelete: 'cascade' }),
		productId: text('product_id')
			.notNull()
			.references(() => products.id, { onDelete: 'cascade' }),
...
export const foundryActivationRequests = sqliteTable('foundry_activation_requests', {
	...
	foundryVersion: text('foundry_version'),
	moduleVersion: text('module_version'),
	status: text('status', { enum: ['pending', 'approved', 'denied', 'expired', 'consumed'] })
		.notNull()
		.default('pending'),
	approvedByUserId: text('approved_by_user_id').references(() => user.id, { onDelete: 'set null' }),
	installationId: text('installation_id').references(() => foundryInstallations.id, {
		onDelete: 'set null'
	}),
