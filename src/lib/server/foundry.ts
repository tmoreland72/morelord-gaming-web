...
export async function validateInstallationToken(d1: D1Database, token: string) {
	const db = getDb(d1);
	...
	const productFeatureRows = await db.select({ key: features.key })
		...
		.where(and(eq(productFeatures.productId, installation.productId), inArray(productFeatures.tier, allowedTiers)));
	...
	await db.update(foundryInstallations).set({ lastValidatedAt: new Date(), updatedAt: new Date() }).where(eq(foundryInstallations.id, installation.id));
	return { installationId: installation.id, productSlug: installation.productSlug, label: installation.label, tier, features: featureKeys, validatedAt: new Date(), refreshAfterSeconds: 86_400 };
}
